...
#gf.gera_grafico_cnt_cts(contrato,valorContrato,flts,'2019-1-1','2023-12-1','Mensal','MWh',True)
...
def update_grafico_tabela(n_mes,n_ano,n_mes_ano, n_det_res, n_unidade, start_date, end_date,tipo_contrato_value,submercado_value,leilao_value,status_value,produto_value,vendedor_value,inicio_suprimento_value,fim_suprimento_value,selecao_value,url):
    ...
    if url == '/' or url == '/contratos':
        ...
        if n_mes_ano != None or  n_unidade != None:
            gf.gera_grafico_cnt(contrato,valorContrato,flts,start_date,end_date,mes_ano_atual,mwh_mwm_atual,False)
        else:
        ...
        if n_mes_ano != None or  n_unidade != None:
            gf.gera_grafico_balanco(balanço,start_date,end_date,mes_ano_atual,False)
        else:
        ...
        if n_mes_ano != None or  n_unidade != None:
            gf.gera_grafico_cts(contrato,custosContratos,flts,start_date,end_date,mes_ano_atual,uni_tot_atual,False)
        else:
            gf.gera_grafico_cts(contrato,custosContratos,flts,start_date,end_date,mes_ano_atual,uni_tot_atual,True)
...
def display_page(pathname):
    ...
    if pathname == '/':
        if valorContrato == {}:
            r = requests.get('https://7b12145af545.ngrok.io/valorcontratostest', timeout=300)
        ...
        if balanço == []:
            r = requests.get('https://7b12145af545.ngrok.io/balanco', timeout=300)
            if r.status_code == 200:
                balanço = r.json()
            else:
        ...
        if custosContratos == {}:
            r = requests.get('https://7b12145af545.ngrok.io/custoscontratos', timeout=300)
            if r.status_code == 200:
                custosContratos = r.json()
            else:
                print('Custos falhou')
        if ipca == []:
            r = requests.get('https://7b12145af545.ngrok.io/ipca', timeout=300)
            if r.status_code == 200:
                ipca = r.json()
            else:
                print('IPCA falhou')
